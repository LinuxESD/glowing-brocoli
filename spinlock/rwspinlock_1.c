...
int thread_function1(void *pv)
{
	...
	pr_info("In EmbeTronicX Thread Function1 : writing value\n");
        etx_global_variable++;
...
int thread_function4(void *pv)
{
    ...
    {
        read_lock(&etx_rwlock);
        pr_info("In EmbeTronicX Thread Function4 : Read value %lu\n", etx_global_variable);
        read_unlock(&etx_rwlock);
        msleep(10);
    }
    return 0;
...
int thread_function5(void *pv)
{
    ...
    {
        read_lock(&etx_rwlock);
        pr_info("In EmbeTronicX Thread Function5 : Read value %lu\n", etx_global_variable);
        read_unlock(&etx_rwlock);
        msleep(10);
    }
    return 0;
...
static int __init etx_driver_init(void)
{
	...
	/* Creating Thread 4 */
        ...
        if(etx_thread4) {
            pr_err("Kthread4 Created Successfully...\n");
        } else {
	...
	/* Creating Thread 5 */
        ...
        if(etx_thread5) {
            pr_err("Kthread5 Created Successfully...\n");
        } else {
            pr_err("Cannot create kthread5\n");
             goto r_device;
        }
