...
struct task_struct * etx_thread, *etx_thread2;
...
int thread_function(void *pv)
{
	...
	{
		pr_info("In EmbeTronicX Thread-1 Function %d\n", i++);
		msleep(1000);
	}
...
int thread2_function(void *pv)
{
	...
	while(!kthread_should_stop())
	{
		pr_info("In EmbeTronicX Thread-2 Function %d\n", i++);
		msleep(1000);
	}
...
static int __init etx_driver_init(void)
{
	...
	if(etx_thread)
	{
		...
		pr_info("Kthread 1 Created Successfully...\n");
	}
	else
	{
	...
	etx_thread2 = kthread_create(thread2_function, NULL, "etx thread2");
	if(etx_thread)
	{
